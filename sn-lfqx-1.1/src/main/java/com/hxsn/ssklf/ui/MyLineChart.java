...
public class MyLineChart extends View {
    ...
    final int[] yLabelArray2 = {0,20,40,50,60,80,100};
    final int[] yLabelArray3 = {-10,0,10,20,30,40,50};
    final int[] yLabelArray4 = {0,10000,18000,25000,32000,40000,50000};
    private DecimalFormat decimalFormat = new DecimalFormat("###################.###########");
    private List<Integer> yLabelList;
    private Context context;
    private List<Double> yValList;//y轴数据列表
    private List<Integer> xValList;//x轴数据列表
    ...
    protected void onDraw(Canvas canvas) {
     ...
     //   Log.i("MyLineChart",",yStart="+yStart);
        ...
        int  yLength = yTotalLength/6;                      //y轴分段长度
        //画x轴数值 和 x网格
        ...
        if(!isHaveData){//没有数据的时候
            ...
            xLength = xValList.size()==0?0:xTotalLength/xValList.size()-1;
        ...
        paint.setStrokeWidth(1);
        for(int i=0; i<7; i++){
            int size = String.valueOf(yLabelList.get(i)).length()*10;
        ...
        if(isHaveData && yValList.size() > 0){
            ...
            for(int i=0; i<xValList.size(); i++){
                Log.i("MyLineChart","index="+i+"xValList.size()="+xValList.size());
                ...
                if(!yValList.get(i).isNaN()){
                    ...
                    if(position < 0 ){//超下限时
                        ...
                        k = i;
                        pointY = yStart-yLength*6-10;//-step;
                        pointX = xStart + (i * xLength);
                        paint.setColor(COLOR_WORNING);
                        paint.setTextSize(TEXT_TITLE_SIZE);
                        if(type == 4){
                            paint.setTextSize(TEXT_TITLE_SIZE/2);
                        }
                        ...
                        int step = yDownLabel-yUpLabel==0?0:(int) (yLength* (yValList.get(i)-yUpLabel)/(yDownLabel-yUpLabel));
                        pointY = yStart-yLength*position-step;
                        pointX = xStart + (i * xLength);
                    }
                    ...
                    if(!yValList.get(i).isNaN()){
                        ...
                        canvas.drawCircle(pointX, pointY, 8, paint);
                    }
                }
            }
        }
    ...
    private int getRelative(int a){
        ...
        return windowHeight==0?0:a*this.getHeight()/windowHeight;
    }
