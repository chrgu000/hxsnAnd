            ...
            case R.id.btn_submit:
                ...
                if (curPsw.length() != 0) {
                    if (newPsw.equals(verifyPsw)) {
                        if(newPsw.length()<6 ){
                            UpdateUtil.show(this, "密码长度不少于6位");
                            break;
                        }else if(newPsw.length()>32){
                            UpdateUtil.show(this, "您的密码设的太长了");
                            break;
                        }
                        ...
                        params.put("oldpwd",curPsw);
                        params.put("newpwd",newPsw);

                        new AndHttpRequest(this) {
                            @Override
                            public void getResponse(String response) {
                                UpdateUtil.show(PasswordEditActivity.this, "密码修改成功请重新登录");
